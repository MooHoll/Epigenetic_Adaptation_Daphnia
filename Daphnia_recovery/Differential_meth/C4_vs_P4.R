## -------------------------------------------------------------------------
## Differential Methylation Between Control and Pesticide GEN4
## -------------------------------------------------------------------------
...
file.list <- list("36_01_C4_CpG.txt","36_01_P4_CpG.txt",
                  "53_01_C4_CpG.txt",
                  "53_01_P4_CpG.txt",
                  "54_01_C4_CpG.txt","54_01_P4_CpG.txt",
                  "6_2_C4_CpG.txt","6_2_P4_CpG.txt",
                  "7_5_C4_CpG.txt",
                  "7_5_P4_CpG.txt",
                  "9_20_C4_CpG.txt","9_20_P4_CpG.txt")


raw_data <- methRead(file.list,
                     sample.id = list("36_01_C4","36_01_P4",
                                      "53_01_C4",
                                      "53_01_P4",
                                      "54_01_C4","54_01_P4",
                                      "6_2_C4","6_2_P4",
...
nrow(methBase_ob) #3938 sites left (increased with normalisation to 4096)
write.table(methBase_ob, file="C4_vs_P4_objectmethbase.txt", quote=F, row.names = F, sep = '\t')

## -------------------------------------------------------------------------

# Generate some quick plots which can be fixed up later
pdf("C4_vs_P4_correlation.pdf")
getCorrelation(subset_methBase,plot=TRUE)
dev.off()

pdf("C4_vs_P4_cluster.pdf")
clusterSamples(subset_methBase, dist="correlation", method="ward", plot=TRUE)
dev.off()

pdf("C4_vs_P4_PCA.pdf")
...
write.csv(diff_meth, file="C4_vs_P4_all_tested_meth_sites_MSCfilter.csv")

diff_meth_10 <- getMethylDiff(diff_meth, difference=10, qvalue=0.05)
nrow(diff_meth_10) #66 (increased with normalisation to 107!)
write.csv(diff_meth_10, file="C4_vs_P4_DMRs_min10percentDiff_qval0.05_MSCfilter.csv")
...
write.table(output_dedup, file="C4_vs_P4_diff_Meth_Genes_MSCfilter_with_geneID.csv",
            sep="\t", row.names=F, quote=F)

write.csv(output_dedup$geneID, file="C4_vs_P4_list_DMGs_MSCfilter.csv")

# Forgot to remove rows with NA to see how many CpGs in gene and not
library(readr)
data1 <- read.csv("C4_vs_P4_diff_Meth_Genes_MSCfilter_with_geneID.csv", sep="\t")
data2 <- data1[!is.na(data1$geneID),]
nrow(data1)
nrow(data2)
write.table(data2, file="C4_vs_P4_diff_Meth_Genes_MSCfilter_with_geneID.csv",
            sep="\t", row.names=F, quote=F)

# 101 genes and 7 not in genes