## -------------------------------------------------------------------------
## Differential Methylation Between Control and Pesticide GEN6
## -------------------------------------------------------------------------
...
file.list <- list("36_01_C6_CpG.txt","36_01_P6_CpG.txt",
                  "53_01_C6_CpG.txt",
                  "53_01_P6_CpG.txt",
                  "54_01_C6_CpG.txt","54_01_P6_CpG.txt",
                  "6_2_C6_CpG.txt","6_2_P6_CpG.txt",
                  "7_5_C6_CpG.txt",
                  "7_5_P6_CpG.txt",
                  "9_20_C6_CpG.txt","9_20_P6_CpG.txt")


raw_data <- methRead(file.list,
                     sample.id = list("36_01_C6","36_01_P6",
                                      "53_01_C6",
                                      "53_01_P6",
                                      "54_01_C6","54_01_P6",
                                      "6_2_C6","6_2_P6",
...
write.csv(diff_meth, file="C6_vs_P6_all_tested_meth_sites_MSCfilter.csv")

diff_meth_10 <- getMethylDiff(diff_meth, difference=10, qvalue=0.05)
nrow(diff_meth_10) #66 (increased with normalisation to 107!)
write.csv(diff_meth_10, file="C6_vs_P6_DMRs_min10percentDiff_qval0.05_MSCfilter.csv")
...
write.table(output_dedup, file="C6_vs_P6_diff_Meth_Genes_MSCfilter_with_geneID.csv",
            sep="\t", row.names=F, quote=F)

write.csv(output_dedup$geneID, file="C6_vs_P6_list_DMGs_MSCfilter.csv")

# Forgot to remove rows with NA to see how many CpGs in gene and not
library(readr)
data1 <- read.csv("C6_vs_P6_diff_Meth_Genes_MSCfilter_with_geneID.csv", sep="\t")
data2 <- data1[!is.na(data1$geneID),]
nrow(data1)
nrow(data2)
write.table(data2, file="C6_vs_P6_diff_Meth_Genes_MSCfilter_with_geneID.csv",
            sep="\t", row.names=F, quote=F)

# 37 genes and 7 not in genes