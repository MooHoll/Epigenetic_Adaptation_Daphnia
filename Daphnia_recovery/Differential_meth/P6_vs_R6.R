## -------------------------------------------------------------------------
## Differential Methylation Between Pesticide and Recovery 
## -------------------------------------------------------------------------
...
file.list <- list("36_01_P6_CpG.txt","36_01_R6_CpG.txt",
                  "53_01_P6_CpG.txt",
                  "53_01_R6_CpG.txt",
                  "54_01_P6_CpG.txt","54_01_R6_CpG.txt",
                  "6_2_P6_CpG.txt","6_2_R6_CpG.txt",
                  "7_5_P6_CpG.txt",
                  "7_5_R6_CpG.txt",
                  "9_20_P6_CpG.txt","9_20_R6_CpG.txt")


raw_data <- methRead(file.list,
                     sample.id = list("36_01_P6","36_01_R6",
                                      "53_01_P6",
                                      "53_01_R6",
                                      "54_01_P6","54_01_R6",
                                      "6_2_P6","6_2_R6",
...
write.csv(diff_meth, file="P6_vs_R6_all_tested_meth_sites_MSCfilter.csv")

diff_meth_10 <- getMethylDiff(diff_meth, difference=10, qvalue=0.05)
nrow(diff_meth_10) #66 (increased with normalisation to 107!)
write.csv(diff_meth_10, file="P6_vs_R6_DMRs_min10percentDiff_qval0.05_MSCfilter.csv")
...
write.table(output_dedup, file="P6_vs_R6_diff_Meth_Genes_MSCfilter_with_geneID.csv",
            sep="\t", row.names=F, quote=F)

write.csv(output_dedup$geneID, file="P6_vs_R6_list_DMGs_MSCfilter.csv")

# Forgot to remove rows with NA to see how many CpGs in gene and not
library(readr)
data1 <- read.csv("P6_vs_R6_diff_Meth_Genes_MSCfilter_with_geneID.csv", sep="\t")
data2 <- data1[!is.na(data1$geneID),]
nrow(data1)
nrow(data2)
write.table(data2, file="P6_vs_R6_diff_Meth_Genes_MSCfilter_with_geneID.csv",
            sep="\t", row.names=F, quote=F)

# 40 genes and 13 not in genes