...
file_name <- list.files("./peak_counts", pattern="*peak_coverage.txt")
...
sample_info$population <- c(rep("CWP",12), rep("EP", 26), rep("CWP",17), rep("PR",7),
                            rep("CWP", 3), rep("PR",17), rep("PP",9), rep("PR",3),
                            rep("PP", 6), rep("PR", 2), rep("PP",3), rep("PR",3),
                            rep("PP", 12), rep("PR",2))

write.table(sample_info, file="ATAC_sample_matrix.txt", sep='\t', quote = F, col.names = T, row.names = T)
...
files <- file.path("./peak_counts", paste0(sample_info$file_name))
...
rownames(all) <- all$peak
countmatrix <- all[,-1]

write.table(countmatrix, file="ATAC_count_matrix.txt", sep='\t', quote = F, col.names = T, row.names = T)
